   ...
   def planning(self):
      ...
      while True:
         ...
         for i in range(len(self.goals)):
            ...
            check = self.checkR((openArr[minCount].x, openArr[minCount].y, openArr[minCount].a), (goal.x, goal.y, goal.a))
            ...
            if check[0]:
               ...
               if abs((check[4] + openArr[minCount].a) - (self.goals[i][2])) < 0.05 and openArr[minCount].a != 0 and check[5] < 500: 
                  arc = self.getArc(check[2], check[1], 10, check[3], openArr[minCount], img, check[1])
                  ch = self.checkArc(arc, check, openArr[minCount], img, width, height)
                           
                  if ch :
                     rotatedX = 17*math.cos(openArr[minCount].a + math.pi/2) + openArr[minCount].x
                     rotatedY = 17*math.sin(openArr[minCount].a + math.pi/2) + openArr[minCount].y
                     self.count = openArr[minCount].i
...
class CreateLocalRrt(Node):
   ...
   def timer_path_out(self):
      ...
      for i in range(int(len(self.pathArr)/4)):
         ...
         if self.IsPointInCircle(self.odomX, self.odomY, self.pathArr[i][0], self.pathArr[i][1], 0.1) and i < len(self.pathArr)-1 and i - self.cPoint < 2:
            #print("delete")
