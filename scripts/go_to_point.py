...
class GoToPoint(Node):
  ...
  def on_timer(self):
    self.goalPosX = self.pathArr[self.i][0]
    self.goalPosY = self.pathArr[self.i][1]
    engleGoaltoAuto = -1
    VecGoalX = 1.0
    VecGoalY = 1.0
    marker = Marker()
    ...
    if IsPointInCircle(self.x, self.y, marker.pose.position.x, marker.pose.position.y, 0.2):
      self.tergetEngle = 0.0
      linearSp = 0.0
      self.i += 1
